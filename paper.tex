...
\begin{figure}[t!]
\centering
\subfigure[]{\label{fig:pioggia} \includegraphics[width=0.3\linewidth]{Immagini/pioggia}}
\subfigure[]{\label{fig:neve} \includegraphics[width=0.3\linewidth]{Immagini/neve}}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{Immagini/sequenze}
\caption{Sequences taken from webcams: \textbf{(a)} no tampering events; \textbf{(b)} defocus event on 4-th and 5-th frames, created using gaussian filtering; \textbf{(c)} displacement event on 4-th and 5-th frames, created using concatenation between similar sequences.}
\label{fig:sequences}
\end{figure}
...
\caption{Example of displacement. On the left side of the image is reported the behavior of the average value of the luma, while on the right side there is its detrending.
The event comes at frame $157$ and the corresponding value of $l(t)$ is depicted with a red star. 
\textbf{(a)} Considering the average value of the luma in the whole scene. 
\textbf{(b)} and \textbf{(c)} Considering the average value of the luma in a specific region.}
...
We acquired two datasets: the first dataset contains  sequences of  frames taken from webcams monitoring urban areas (e.g., see Figure \ref{fig:sequences}(a)). 
The second dataset was recorded using a Raspberry Pi Model B+ \cite{raspberry} mounting a camera module. 
In the first dataset tampering was synthetically generated by blurring frames (see, Figure \ref{fig:sequences}(b)) or concatenating frames from different sequences depicting similar environments (see Figure \ref{fig:sequences}(c)). 
...
At last we have considered $9$ different scenarios, with $10$ displacement events and $10$ blurring events each, for a total of $180$ tampering events.%\gi{una footnote dicendo sequences can be released upon request?}
...
ROC curves and the AUC in Figure~\ref{fig:ROCdisplacement} caption indicate that camera displacements can be effectively detected. However, the best performance are achieved by Algorithm \ref{alg:DISPL} the (\emph{Frame-Difference} curve has not been reported because it was below these). This result indicate that separately monitoring the luma over image regions is an effective strategy and that the regions have to be carefully designed. Please note that, for the visualization sake, these ROC curves have been reported in a limited range of x and y axis. While the improvement might seem negligible we observe that Algorithm~\ref{alg:DISPL}, for having a recall of $98\%$, achieves $0.3\%$ false positive rates (i.e., 1-SPECIFICITY) than $0.7\%$ of the whole image solution or $1\%$ of the Voronoi regions solution.
Having a solution which halves the false positives is important in a low-power context, considering the high number of frames to be processed.\\
