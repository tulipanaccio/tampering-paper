...
Let us denote by $z_t$ the frame acquired at time $t$, which we model as
\begin{equation}
\label{eq:observationModel}
z_t(x)=\mathcal{D}_t[y_t](x) \quad \forall x \in X
\end{equation}
where $\mathcal{D}_t$ denotes the degradation operator that transforms the original image $y_t$ in the frame $z_t$ which we acquire; $X \subset \mathbb{Z}$ denotes the regular pixels grid and $x$ is a vector indicating the pixel coordinates. As far as there is no tampering attacks/events,
\begin{equation}
\label{eq:no_tampering}
\mathcal{D}_t[y_t](x) = y_t(x) + \eta_t(x) \quad \forall x \in X
\end{equation}
where $\eta_t$ is a random variable accounting for all the noise sources (thermal, quantization, photon-counting). In normal conditions, all the images $y_t$ (thus also the frames $z_t$) might be different in content but are acquired from the same viewpoint and the same camera orientation.

When at time $\tau^*$ an external disturbance over the camera lens introduces a defocus, the image $y_t$ is blurred by spatially variant operator:
\begin{equation}
\label{eq:model_defocus}
\mathcal{D}_t[y_t](x) = \int_{\mathcal{X}}y(s)h_t(x,s)ds\, + \eta_t(x) \quad \forall x \in X, t \leq \tau^*
\end{equation}
...
%\end{equation}
A camera displacement at frame $\tau^*$ is modeled as 
\begin{equation}
...
\end{array}\right. ,
\end{equation}
where $w_t$ is acquired from a different viewpoint and/or camera orientation than $y_t$. 

The proposed tampering-detection algorithm analyzes a sequence of frames $\{z_t\}_t$ and detect frames at time $\tau^*$ that correspond to a tampering like \eqref{eq:model_defocus} or \eqref{eq:model_displacement}.
...
The other part of this dataset is made of frame sequences taken from webcams available on the Internet. 
In these sequences there were a few events of displacements or defocus, but to have a larger set of tampering events we have simulated them.
In particular we have used gaussian filtering for simulating defocus events, and concatenation of similar sequences for simulating displacement events.\\
