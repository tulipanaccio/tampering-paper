...
\begin{abstract}
A desirable feature for smart cameras is the ability to autonomously detect any tampering event that would prevent a clean view over the monitored scene. No matter whether tampering is due to atmospheric phenomena (e.g., few water drops over the camera lens) or to malicious events (e.g., the device displacement), these have to be promptly detected to possibly activate countermeasures. Tampering detection becomes particularly challenging in battery-powered cameras, where it is not possible to acquire images at video-like frame-rates, nor use sophisticated image-analysis algorithms. 
...
One of the most critical issues in smart cameras, used as nodes in wireless multimedia sensor networks (WMSN), is the automatic detection of events that could 

In general, techniques used to identify these type of events are called tampering detection algorithms.

A desirable feature for smart cameras is the ability to autonomously detect any tampering event that could impair a clean view over the monitored scene. No matter whether tampering events are due to atmospheric phenomena (e.g., water drops over the camera lens) or malicious attacks (e.g. displacement of the device), these have to be promptly detected and reported. 
