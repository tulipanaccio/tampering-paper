...
The proposed tampering-detection algorithm relies on two indicators that are very easy to compute: the average intensity (also refer to \emph{luma} and denoted by $l$) and the average norm of the gradient (denoted by $g$). To analyze an incoming frame, we compute these indicators at first and then detect possible tampering attacks by determining whether these falls outside a confidence interval defined during an initial training phase.
