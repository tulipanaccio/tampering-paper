...
"Algo 1 on one region" is $99.89\%$, while the AUC obtained by "Total" (on the whole 
image) is $99.65\%$.
\textbf{(b)} ROC curves relative to displacement detection based on Luma: the AUC obtained by 
"Algo 1 on one region" is $98.44\%$, while the AUC obtained by "Total" (on the whole 
image) is $84.07\%$.
\textbf{(c)} ROC curves relative to blurring detection based on Gradient: as opposed to \textbf{(a)} and 
\textbf{(b)}, "Algo 1 on one region" (AUC $98.4\%$) is outperformed by "Total" (AUC $99.13\%$).
