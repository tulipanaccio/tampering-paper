...
The most computationally demanding operations of Algorithm \ref{alg:DISPL} consists in computing of the indicators, in particular $g$. Assuming that $f_v$ and $f_h$ in~\eqref{eq:normaGradiente} are Sobel filters, this requires $34$ operations per pixels\footnote{Execution can be accelerated whether hardware implementations of the FFT transform are available}. Computing $l$ and $d$ requires 1 and 3 operations per pixel, respectively, while detecting outliers in the indicators requires two comparisons per frame. Thus, monitoring regions instead of the whole image has a negligible impact over the overall computational complexity. Algorithm~\ref{alg:DISPL} has a very low memory requirement: to compute $d$ it is necessary to store a single frame, while other indicators can be computed online. Segmentation is executed only during the initial configuration and can be performed on an external device connected to the smart camera. As such, Algorithm~\ref{alg:DISPL} can be properly executed on low-power and ultra low-power cameras.
...
Experiments are meant to assess the advantages of using a segmentation in a tampering-detection framework like Algorithm~\eqref{alg:DISPL}. In particular, to assess the importance of partitioning the scene we compare Algorithm~\eqref{alg:DISPL} against a solution monitoring the indicators on the whole image (\emph{Total} in Figure~\ref{fig:ROC}). To show that regions have to be defined depending on the image content, we consider also Algorithm~\eqref{alg:DISPL} when the $K$ regions are obtained by clustering $[r(x), c(x)]$, which yields a Voronoi partitioning of the scene (\emph{Voronoi} in Figure~\ref{fig:ROC}). For both regions defined as in Section~\ref{subsec:Segmentation} and Voronoi regions, we test the two extreme cases where $\Gamma_l = 1$ and $\Gamma_l = K-1$ ($\Gamma_d = 1$ and $\Gamma_d = K-1$).

\subsubsection{Datasets}
We recorded $8$ sequences from webcams monitoring different urban areas and we synthetically introduced $5\%$ of tampered frames in each. In each sequence we have cropped the central region, removing 50 rows from the top and bottom, and 50 columns from the left and right of the image. Blurring was obtained by convolution against a Gaussian kernels (as in Figure \ref{fig:sequences}(a)), while displacements were simulated by moving the central cropping regions of a random shift between 20 and 50 pixels in each direction (see Figure \ref{fig:sequences}(b)).  Overall, the dataset contains $24410$ frames, in which we've inserted $5\%$ of displacement events ($1224$ frames) and $5\%$ of blurring events ($1224$ frames).\footnote{Sequences can be released upon request.}
\gi{image sizes? blur kernel? quanti pixel il displacemnet?}
